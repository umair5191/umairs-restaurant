...
router.post('/registered', [check('email').isEmail(), check('password').isLength({ min: 10 }), check('first').notEmpty(), check('last').notEmpty(), check('username').notEmpty()], function (req, res, next) {
    ...
    if (!errors.isEmpty()) {
        res.render('register/register_error.ejs');  // If there are errors, user is informed and prompted to try again
    }
    else {
        ...
        bcrypt.hash(plainPassword, saltRounds, function(err, hashedPassword) { // Hashing the password
            ...
            db.query(sqlquery, [username, first, last, email, hashedPassword], (err, result) => {
                ...
                res.render('register/user_registered.ejs', {first}); // If successful, user is informed that they have been registered
            })                                                                           
        })
    }
})  
...
router.get('/login', function (req, res, next) {
    if (req.session.userId) { // Checking if user is already logged in
        res.render('login/login_again.ejs', {username: req.session.userId}); // If user is already logged in, they are informed and prompted to logout
    }
    else {
...
router.post('/loggedin', function (req, res, next) {
    ...
    db.query(sqlquery, [req.sanitize(req.body.username)], (err, result) => { // sanitizing the username to prevent harmful attacks
        ...
        if (result.length == 0) {
            return res.render('login/login_error.ejs'); // If username is not found, user is informed and prompted to try again
        }
        ...
        bcrypt.compare(req.sanitize(req.body.password), hashedPassword, function(err, result) { // sanitizing the password to prevent harmful attacks
            if (result == true) {
                ...
                res.render('login/loggedin.ejs', {username: req.sanitize(req.body.username)}); // If password is correct, user is informed that they are logged in
            }
            else {
                res.render('login/login_error.ejs'); // If password is incorrect, user is informed and prompted to try again
            }
        })
    })
})
...
router.get('/logout', function (req, res, next) {
    if (!req.session.userId) { // Checking if user is logged in
        res.render('logout/logout_error.ejs'); // If user is not logged in, they are informed that they cannot log out and prompted to login
    }
    else {
        req.session.destroy(); // If user is logged in, their session is destroyed
        res.render('logout/loggedout.ejs'); // User is informed that they have been logged out
    }
})
